...
struct NewIntegratorOmega : public ::OMNI::MagnusIntegratorSO3R3{

    NewIntegratorOmega(const unsigned int t_number_of_Chebyshev_points,
                    std::shared_ptr<const ::CROSP::strain_parameterisation_stack::StrainParameterisationStack> t_strain_parameterisation_stack)
        : ::OMNI::MagnusIntegratorSO3R3(t_number_of_Chebyshev_points),
          m_strain_parameterisation_stack(t_strain_parameterisation_stack),
          m_interpolator(::Chebyshev::ChebyshevInterpolator(t_number_of_Chebyshev_points, this->m_quadrature_points))
    {
        const auto zeros = Eigen::Vector3d::Zero();
...
//            const unsigned int stack_index = t_Chebyshev_point*4 + t_quadrature_point+1;
            ...
            this->m_K1_stack.push_back( m_strain_parameterisation_stack->m_K_stack->at(point*4 + 1) );
            this->m_K2_stack.push_back( m_strain_parameterisation_stack->m_K_stack->at(point*4 + 2) );
            this->m_K3_stack.push_back( m_strain_parameterisation_stack->m_K_stack->at(point*4 + 3) );


            this->m_b1_stack.push_back( zeros );
            this->m_b2_stack.push_back( zeros );
            this->m_b3_stack.push_back( zeros );
        }
    }
...
//            const unsigned int stack_index = t_Chebyshev_point*4 + t_quadrature_point+1;
            ...
            this->m_b1_stack[point] = m_strain_parameterisation_stack->m_dot_K_stack->at(point*4 + 1);
            this->m_b2_stack[point] = m_strain_parameterisation_stack->m_dot_K_stack->at(point*4 + 2);
            this->m_b3_stack[point] = m_strain_parameterisation_stack->m_dot_K_stack->at(point*4 + 3);
        }
//        const unsigned int stack_index = t_Chebyshev_point*4 + t_quadrature_point+1;
...
int main(int argc, char *argv[])
{
    ...
    NewIntegratorOmega new_Omega_integrator(number_of_Chebyshev_points, m_strain_parameterisation_stack);
    ...
    const Eigen::VectorXd dot_q  = 100*Eigen::VectorXd::Random(coordinated_dimension);
    const Eigen::VectorXd ddot_q = 1000*Eigen::VectorXd::Random(coordinated_dimension);
...
//          0.75, -2.5, 0.25, 0.3333;
    ...
    const Eigen::Vector3d Omega_X0 = Eigen::Vector3d::Zero();
    ...
    ::benchmark::RegisterBenchmark("Update Param", [&](::benchmark::State &t_state){
        ...
        m_strain_parameterisation_stack->copy_stack = true;
    ...
    ::benchmark::RegisterBenchmark("Integration Omega", [&](::benchmark::State &t_state){
        ...
        while(t_state.KeepRunning()){
            ...
            Omega_integrator->integrate(R_X0);
    ...
    ::benchmark::RegisterBenchmark("New Update Param", [&](::benchmark::State &t_state){
        ...
        m_strain_parameterisation_stack->copy_stack = false;
    ...
    ::benchmark::RegisterBenchmark("New Integration Omega", [&](::benchmark::State &t_state){
        ...
        while(t_state.KeepRunning()){
            ...
            new_Omega_integrator.computesCoefficientsMatricesAtQuadraturePoints();
            new_Omega_integrator.updateParametersVector();
            new_Omega_integrator.integrate(Omega_X0);
...
////        std::cout << "error orientation : \n" << integrate_rotation_matrix->getStateAtPoint(i).transpose()*integrate_SO3->getStateAtPoint(i) << "\n\n";
////    }

//    std::cout << "\n\n\n\nOMEGA\n\n\n\n";

//    Omega_integrator->computesCoefficientsMatricesAtQuadraturePoints();
//    Omega_integrator->integrate(Omega_X0);
...
//    for(unsigned int i=0; i<number_of_Chebyshev_points; i++){
//        std::cout << "error Omega : \n" << new_Omega_integrator.getStateAtPoint(i) - Omega_integrator->getStateAtPoint(i) << "\n\n";
//    }
