...
Const.dim_base_k = [5 5 5 0 0 0]';%load("../test_different_modes/ne_stack.csv");
% Calcul de la taille de q 
Const.dim_base   = Config.V_a'*Const.dim_base_k;


% Valeur de la gravité
Const.Gamma_g = 0;%9.81;
...
Const.B = [eye(3); zeros(3)];%load("../test_different_modes/B.csv");
Const.B_bar = [zeros(3); eye(3)];%load("../test_different_modes/B_bar.csv");
...
eta_0 = [0 0 0 0 0 0]';%[.5 .25 -.33 0 0 0]'; % vitesse de la tête
...
Const.q = load("q.csv");
Const.q_dot = load("dot_q.csv");
Const.q_dot_dot = load("ddot_q.csv");
...
Q_cpp = load("Quaternion.csv");
% r_cpp = load("../test_different_modes/r_stack.csv");
Omega_cpp = load("Omega.csv");
V_cpp = load("V.csv");
dot_Omega_cpp = load("dot_Omega.csv");
dot_V_cpp = load("dot_V.csv");
...
% %   Reshape
% Omega_stack = [
%   flip(Omega_cpp(       1:rows))'
%   flip(Omega_cpp(  rows+1:2*rows))'
%   flip(Omega_cpp(2*rows+1:end))'
% ];
...
Omega_stack_ref = eta_X(1:3, :);
error_Omega = Omega_cpp - Omega_stack_ref;
