...
\For{$i \in \left[0, N_c - 1 \right)$}{\;
	...
	$begin = Chebyshev\_points \left[ (N_c-1) - i \right]$\;
    	$end = Chebyshev\_points \left[ (N_c-1) - (i + 1) \right]$\;\;
...
\For{$i \in \left[0, N_c - 1 \right)$}{\;
	...
	$\dot{\xi}\_stack[i] = \{\begin{matrix} \dot{\xi}_1 & \dot{\xi}_2 & \dot{\xi}_3 \end{matrix} \}$\;\;
	
	$\ddot{\xi} _1 = B\Phi\_stack[i]_{(x_1)} \ddot{q} $	\;
	$\ddot{\xi} _1 = B\Phi\_stack[i]_{(x_2)} \ddot{q} $	\;
	$\ddot{\xi} _1 = B\Phi\_stack[i]_{(x_3)} \ddot{q} $	\;
	$\ddot{\xi}\_stack[i] = \{\begin{matrix} \ddot{\xi}_1 & \ddot{\xi}_2 & \ddot{\xi}_3 \end{matrix} \}$\;\;
...
\For{$i \in \left[0, N_c - 1 \right)$}{
	$\textit{Get curvature from strain field at quadrature point}$\;
	$\begin{bmatrix}K_1 & K_2 & K_3 \end{bmatrix} \leftarrow \begin{bmatrix}\xi_1 & \xi_2 & \xi_3 \end{bmatrix}$\;\;
...
\For{$i \in \left[0, N_c - 1 \right)$}{
	...
	$R\_stack[i+1] = R\_stack[i] \cdot  stack\_e^{\Omega^{[6]}}[i]$\;
...
Note that at the end of the algorithm \ref{algo:magnus} we use a different formulation from the one shown in \eqref{eq:recursion}, as in this case we directly move from the beginning of the integration domain to its end.
