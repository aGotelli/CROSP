function [QX, rX, eta_X, eta_dot_X, Lambda_X, Qa_X, ...
    Delta_zeta_X_stack, Delta_eta_X_stack, Delta_eta_dot_X_stack, Delta_Lambda_X_stack, Delta_Qa_X_stack, J] = TIDM_spectral_output_all_data(qn_0,rn_0,q_0,r_0,eta_0,eta_dot_0,a,b,time,Const,Config)
...
Delta_zeta_X_stack = [];
Delta_eta_X_stack = [];
Delta_eta_dot_X_stack = [];
Delta_Lambda_X_stack = [];
Delta_Qa_X_stack = [];

% Définition des noeuds
N_noeuds = 31; % nombre de noeuds
...
%     Xi(:,it_x) = Const.B*Xi_a +  Const.B_bar*Const.Xi_c + Const.B*Const.Xi_0;
    ...
    xi_c = [0 0 0 1 0 0]';
    Xi(:,it_x) = Const.B*Xi_a +  xi_c + Const.B*Const.Xi_0;
...
for it_D_q = 1:Const.dim_base
    ...
    Delta_zeta_X_stack(:, :, it_D_q) = Delta_psi_X;
...
%     Delta_eta_0 = [[a*T_SO3_m1_(Theta),zeros(3)];[hat_(eta_0(4:6)),a*eye(3)]]*Delta_psi_0;
    ...
    Delta_eta_X_stack(:, :, it_D_q) = Delta_eta_X;
...
%     Delta_eta_dot_0 = [[b*T_SO3_m1_(Theta),zeros(3)];[hat_(eta_dot_0(4:6)+hat_(eta_0(1:3))*eta_0(4:6))+a*hat_(eta_0(4:6))*T_SO3_m1_(Theta)-hat_(eta_0(1:3))*hat_(eta_0(4:6)),b*eye(3)-a*hat_(eta_0(1:3))]]*Delta_psi_0;
    ...
    Delta_eta_dot_X_stack(:, :, it_D_q) = Delta_eta_dot_X;
    % calcul de Delta_Lambda
    M_cal = Const.M_cal;

    for it_x = 1:N_noeuds
        R = quaternion_to_matrice(QX(:,it_x));
        Delta_F_bar(:,it_x)= [zeros(3,1);hat_(Delta_psi_X(:,it_x))'*R'*[0;   0; -Const.rho*Const.Aire*Const.Gamma_g]];
    end
    ...
    Delta_Lambda_X_stack(:, :, it_D_q) = Delta_Lambda_X;
    ...
    Delta_Qa_X_stack(:, :, it_D_q) = Delta_Qa_X;
