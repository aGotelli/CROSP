...
#include "CROSP/numerical_integrators/magnus_expansion/magnus_idm_integrators/magnus_idm_integrators.hpp"
...
IntegratorRotationMatrix::IntegratorRotationMatrix(const unsigned int t_number_of_Chebyshev_points,
...
Eigen::Matrix3d IntegratorRotationMatrix::computeCofficientsMatrixAtQuadraturePoint(const unsigned int t_Chebyshev_point,
                                                                                                const unsigned int t_quadrature_point)
{
    const auto index = t_Chebyshev_point*m_number_of_quadrature_points + t_quadrature_point;

    return m_strain_parameterisation_stack->m_hat_K_stack->at(index);
}



Eigen::Matrix3d IntegratorRotationMatrix::recursiveExponentialMapping(const Eigen::Matrix<double, 3, 3> &t_previous_state,
                                                                                  const MatrixDd &t_exp_Omega)
{
    return t_previous_state * t_exp_Omega;
}


Eigen::Matrix3d IntegratorRotationMatrix::exponentialOmega(const MatrixDd &t_Omega)
{
    return ::LieAlgebra::expRodigues( t_Omega );
}


void IntegratorRotationMatrix::postProcessInterpolation()
{

    Eigen::Quaterniond q;
    for(unsigned int point=0; point<this->m_quadrature_points.size(); point++){
...
Eigen::MatrixXd IntegratorRotationMatrix::getStateAtQuadraturePoint(const unsigned int t_quadrature_point)const
{
...
MagnusIDMIntegrators::MagnusIDMIntegrators(std::shared_ptr<const strain_parameterisation_stack::StrainParameterisationStack> t_strain_parameterisation_stack,
                               std::shared_ptr<const rod_properties::RodProperties> t_rod_properties)
    : m_strain_parameterisation_stack( t_strain_parameterisation_stack ),
      m_rod_properties(t_rod_properties)
{}
...
}   //  namespace CROSP::numerical_integrators::spectral_method::magnus_idm_integrators
