...
#include "CROSP/numerical_integrators/spectral/idm_integrators.hpp"
...
PositionIntegrator::PositionIntegrator(std::shared_ptr<const ParameterisationStack> t_parameterisation_stack,
...
AngularVelocityIntegrator::AngularVelocityIntegrator(std::shared_ptr<const ParameterisationStack> t_parameterisation_stack,
                                                     const double &t_upper_integration_limit,
                                                     const Eigen::Vector3d &t_initial_condition)
    : OSNI::ODEAb(3,
                  ::Chebyshev::INTEGRATION_DIRECTION::FORWARD,
...
LinearVelocityIntegrator::LinearVelocityIntegrator(std::shared_ptr<const ParameterisationStack> t_parameterisation_stack,
...
AngularAccelerationIntegrator::AngularAccelerationIntegrator(std::shared_ptr<const ParameterisationStack> t_parameterisation_stack,
                                                             std::shared_ptr<const OSNI::ODESolverInterface> t_angular_velocity_integrator,
                                                             const double &t_upper_integration_limit,
                                                             const Eigen::Vector3d &t_initial_condition)
    : OSNI::ODEAb(3,
...
LinearAccelerationIntegrator::LinearAccelerationIntegrator(std::shared_ptr<const ParameterisationStack> t_parameterisation_stack,
                                                           std::shared_ptr<const OSNI::ODESolverInterface> t_angular_velocity_integrator,
...
GeneralisedForcesIntegrator::GeneralisedForcesIntegrator(std::shared_ptr<const ParameterisationStack> t_parameterisation_stack,
...
Eigen::VectorXd GeneralisedForcesIntegrator::computerParametersVectorAtPoint(const unsigned int t_point)
{
    ...
    const Eigen::VectorXd b_at_point = -m_parameterisation_stack->m_map_to_strain_stack.at(t_point).transpose()*Gamma;
    return b_at_point;
}
...
IDMIntegrators::IDMIntegrators(const strain_parameterisation::StrainParameterisation t_strain_parameterisation,
