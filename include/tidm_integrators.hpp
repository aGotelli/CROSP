...
struct DeltaRotation : public OSNI::ODEAb {
    DeltaRotation(const std::shared_ptr<const std::vector<Eigen::Vector3d>> t_K_stack,
                  const std::shared_ptr<const std::vector<Eigen::Vector3d>> t_Delta_K_stack,
...
struct DeltaPosition : public OSNI::ODEAb {
    ...
    DeltaPosition(const std::shared_ptr<const std::vector<Eigen::Vector3d>> t_K_stack,
                  const std::shared_ptr<const std::vector<Eigen::Vector3d>> t_Lambda_stack,
                  const std::shared_ptr<const std::vector<Eigen::Vector3d>> t_Delta_Lambda_stack,
                  const std::shared_ptr<const DeltaRotation> t_delta_rotation,
                  const unsigned int t_number_of_Chebyshev_points) :    OSNI::ODEAb(4, ::Chebyshev::INTEGRATION_DIRECTION::FORWARD,
                                                                                    t_number_of_Chebyshev_points),
                                                                        m_K_stack(t_K_stack),
...
struct TIDMIntegrators{
    ...
    TIDMIntegrators(const std::shared_ptr<const StrainParameterisation> &t_strain_parameterisation,
                    const std::shared_ptr<const IDMIntegrators> &t_idm_integrators) :   m_strain_parameterisation(t_strain_parameterisation),
                                                                                        m_idm_integrators(t_idm_integrators)
    {}

    TIDMIntegrators(const std::shared_ptr<const StrainParameterisation> t_strain_parameterisation,
                    const std::shared_ptr<const IDMIntegrators> &t_idm_integrators,
    ...
    const std::shared_ptr<const StrainParameterisation> m_Delta_strain_parameterisation;
    ...
    std::shared_ptr<DeltaRotation> m_delta_rotation { std::make_shared<DeltaRotation>( m_strain_parameterisation->m_K_stack,
                                                                                       m_Delta_strain_parameterisation->m_K_stack,
    ...
    std::shared_ptr<DeltaPosition> m_delta_position { std::make_shared<DeltaPosition>( m_strain_parameterisation->m_K_stack,
                                                                                       ...
                                                                                       m_Delta_strain_parameterisation->m_Lambda_stack,
                                                                                       m_delta_rotation,
                                                                                       m_number_of_Chebyshev_points) };
